...
"""
    ...
    def __resolve_inno_setup_compiler(self) -> str:
        """
        Resolve the Inno Setup command-line compiler (ISCC.exe) executable path.
        """
        compiler_path = shutil.which("ISCC")
        if compiler_path:
            return compiler_path

        program_files_variables = [
            "ProgramFiles(x86)",
            "ProgramFiles",
            "ProgramW6432",
        ]
        ...
        search_roots = [
            os.environ.get(variable) for variable in program_files_variables
        ]

        local_app_data = os.environ.get("LOCALAPPDATA")
        if local_app_data:
            search_roots.append(os.path.join(local_app_data, "Programs"))

        for search_root in search_roots:
            if not search_root:
                continue

            for version_folder in ("Inno Setup 7", "Inno Setup 6"):
                candidate = os.path.join(search_root, version_folder, "ISCC.exe")
                if os.path.isfile(candidate):
                    return candidate

        raise FileNotFoundError(
            "Could not find 'ISCC.exe'. Install Inno Setup "
            "(winget install --id JRSoftware.InnoSetup) or add it to PATH."
        )

    def __get_inno_setup_architecture_settings(self) -> tuple[str, str]:
        """
        Get the Inno Setup architecture directives for the current platform.
        """
        if self.architecture == Architecture.X86_64:
            return ("x64compatible", "x64compatible")

        if self.architecture == Architecture.ARM64:
            return ("arm64", "arm64")

        raise EnvironmentError(
            f"Unsupported Inno Setup architecture: {self.architecture.value}"
        )

    def __create_inno_setup_script(
        self, source_folder: str, output_folder: str, output_base_name: str
    ) -> str:
        """
        Generate the Inno Setup script describing the UI installation package.
        """
        icon_path = os.path.join(
            self.workspace, "sokketter-ui", "resources", "icons", "socket-icon.ico"
        )
        license_path = os.path.join(self.workspace, "LICENSE")

        (
            architectures_allowed,
            architectures_install_in_64_bit_mode,
        ) = self.__get_inno_setup_architecture_settings()

        script_contents = f"""; Generated by Build.py, do not edit manually.
[Setup]
AppId={{{{9E2D4F0C-9E5B-4C6A-9F0C-1E2A3B4C5D6E}}
AppName=sokketter
AppVersion={self.version}
AppPublisher=Paul Ergård
AppPublisherURL=https://github.com/morwy/sokketter
AppSupportURL=https://github.com/morwy/sokketter/issues
AppUpdatesURL=https://github.com/morwy/sokketter/releases
DefaultDirName={{autopf}}\\sokketter
DefaultGroupName=sokketter
DisableProgramGroupPage=yes
UninstallDisplayIcon={{app}}\\sokketter-ui.exe
...
ArchitecturesInstallIn64BitMode={architectures_install_in_64_bit_mode}
...
[Icons]
Name: "{{autoprograms}}\\sokketter"; Filename: "{{app}}\\sokketter-ui.exe"
Name: "{{autodesktop}}\\sokketter"; Filename: "{{app}}\\sokketter-ui.exe"; Tasks: desktopicon

[Run]
Filename: "{{app}}\\sokketter-ui.exe"; Description: "{{cm:LaunchProgram,sokketter}}"; Flags: nowait postinstall skipifsilent
"""

        script_path = os.path.join(self.temp_build_output_dir, "sokketter-ui.iss")
        with open(file=script_path, mode="w", encoding="utf-8") as file:
            file.write(script_contents)

        return script_path

    def __validate_inno_setup_package(self, installer_path: str) -> None:
        """
        Validate the Inno Setup package by performing a silent installation into a temporary folder.
        """
        if not os.path.isfile(installer_path):
            raise FileNotFoundError(
                f"Inno Setup package was not created: {installer_path}"
            )

        self.logger.info("Validating Windows UI installation package: %s", installer_path)

        validation_folder = os.path.join(
            self.temp_build_output_dir, "sokketter-ui-installer-validation"
        )
        if os.path.exists(validation_folder):
            shutil.rmtree(validation_folder, ignore_errors=True)

        install_folder = os.path.join(validation_folder, "installed")
        os.makedirs(install_folder, exist_ok=True)

        log_path = os.path.join(validation_folder, "install.log")
        ...
        try:
            self.__execute_command(
                [
                    ...
                    "/NORESTART",
                    "/SP-",
                    f"/DIR={install_folder}",
                    f"/LOG={log_path}",
                ]
            )

            installed_executable = os.path.join(install_folder, "sokketter-ui.exe")
            if not os.path.isfile(installed_executable):
                raise FileNotFoundError(
                    f"Installation package did not deploy the UI executable: {installed_executable}"
                )

            if not os.path.isfile(os.path.join(install_folder, "Qt6Core.dll")):
                raise FileNotFoundError(
                    "Installation package did not deploy the Qt6 runtime libraries."
                )

            uninstallers = glob.glob(os.path.join(install_folder, "unins*.exe"))
            if not uninstallers:
                raise FileNotFoundError(
                    "Installation package did not deploy an uninstaller."
                )
        ...
        self.logger.info(
            "Windows UI installation package validation completed successfully."
        )

    def __package_windows_ui_installer(
        self, sokketter_ui_folder: str, sokketter_ui_zip_folder: str
    ) -> None:
        """
        Package the Windows UI files into an Inno Setup installation package.
        """
        self.logger.info("Starting the packaging of Windows UI installation package.")

        output_base_name = (
            f"sokketter-ui-{self.version}-{self.system.value}-{self.os_version}-"
            f"{self.architecture.value}-setup"
        )

        script_path = self.__create_inno_setup_script(
            source_folder=sokketter_ui_zip_folder,
            output_folder=sokketter_ui_folder,
            output_base_name=output_base_name,
        )

        packing_command = [
            self.__resolve_inno_setup_compiler(),
            script_path,
        ]
        self.__execute_command(packing_command)

        self.__validate_inno_setup_package(
            os.path.join(sokketter_ui_folder, f"{output_base_name}.exe")
        )

        self.logger.info("Windows UI installation package packaged successfully.")
    ...
    def __package_ui(self) -> None:
        ...
        if self.system == System.WINDOWS:
            ...
            self.__package_windows_ui_installer(
                sokketter_ui_folder, sokketter_ui_zip_folder
            )
